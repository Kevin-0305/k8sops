...
func Routers() *gin.Engine {
	...
	PublicGroup := Router.Group("")
	{
		...
		router.InitServerGroupRouter(PublicGroup)
		router.InitProjectRouter(PublicGroup)   //自动构建项目
		router.InitBuildTaskRouter(PublicGroup) // 构建历史
	}
	...
	PrivateGroup.Use(middleware.JWTAuth()).Use(middleware.CasbinHandler())
	{
		...
		router.InitMonitorServerRouter(PublicGroup)          //服务器器监控
		router.InitMonitorWebSSHRouter(PublicGroup)          //webssh连接
		router.InitSharedStorageRouter(PublicGroup)
		k8s.InitConfigMapRouter(PublicGroup)
		k8s.InitNamespaceRouter(PublicGroup)
		k8s.InitSecretRouter(PublicGroup)
		k8s.InitServiceRouter(PublicGroup)
		k8s.InitPodRouter(PublicGroup)
		k8s.InitPersistentVolumeRouter(PublicGroup)
		k8s.InitPersistentVolumeClaimRouter(PublicGroup)
